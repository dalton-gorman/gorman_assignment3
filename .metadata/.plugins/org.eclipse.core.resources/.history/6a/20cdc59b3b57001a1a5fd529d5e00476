...
public class CAI5 {	
	...
	public static int mathSign() {
		SecureRandom numberRandomizer = new SecureRandom();
		
		//when user chooses arithmetic type 5, this randomized the sign for each question
		int mathSign = numberRandomizer.nextInt(4);
		mathSign += 1;
	...
	public static double askQuestion(int difficulty, int problemType) {
		...
		//type 5 all	
			...
			//call random math problem generator
			int mathSign = mathSign();
			
			//mathSign = 1 then produce addition question
			if (mathSign == 1) {
				correctAnswer = nums[0] + nums[1];
				System.out.printf("How much is %.0f plus %.0f?\n", nums[0], nums[1]);
				return correctAnswer;
				
			//mathSign = 2 then produce multiplication question
			} else if (mathSign == 2) {
				correctAnswer = nums[0] * nums[1];
				System.out.printf("How much is %.0f times %.0f?\n", nums[0], nums[1]);
				return correctAnswer;
				
			//mathSign = 3 then produce subtraction question
			} else if (mathSign == 3) {
				correctAnswer = nums[0] - nums[1];
				System.out.printf("How much is %.0f minus %.0f?\n", nums[0], nums[1]);
				return correctAnswer;
				
			//mathSign = 4 then produce division question
			} else if (mathSign == 4) {
				//set answer equal to zero if denominator is 0
				if (nums[1] != 0) {
					correctAnswer = nums[0] / nums[1];
